...
def load_data(session):
    ...
    session.sql(
        "create or replace table national_parks(Country varchar, Abreviation varchar, Capital_City varchar, Continent varchar)").collect()
    ...
    session.sql(
        "copy into national_parks from @demo file_format= (type = csv field_delimiter=',' skip_header=1)").collect()
